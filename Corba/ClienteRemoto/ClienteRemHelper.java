package ClienteRemoto;


abstract public class ClienteRemHelper
{
  private static String  _id = "IDL:ClienteRemoto/ClienteRem:1.0";

  public static void insert (org.omg.CORBA.Any a, ClienteRemoto.ClienteRem that)
  {
    org.omg.CORBA.portable.OutputStream out = a.create_output_stream ();
    a.type (type ());
    write (out, that);
    a.read_value (out.create_input_stream (), type ());
  }

  public static ClienteRemoto.ClienteRem extract (org.omg.CORBA.Any a)
  {
    return read (a.create_input_stream ());
  }

  private static org.omg.CORBA.TypeCode __typeCode = null;
  synchronized public static org.omg.CORBA.TypeCode type ()
  {
    if (__typeCode == null)
    {
      __typeCode = org.omg.CORBA.ORB.init ().create_interface_tc (ClienteRemoto.ClienteRemHelper.id (), "ClienteRem");
    }
    return __typeCode;
  }

  public static String id ()
  {
    return _id;
  }

  public static ClienteRemoto.ClienteRem read (org.omg.CORBA.portable.InputStream istream)
  {
    return narrow (istream.read_Object (_ClienteRemStub.class));
  }

  public static void write (org.omg.CORBA.portable.OutputStream ostream, ClienteRemoto.ClienteRem value)
  {
    ostream.write_Object ((org.omg.CORBA.Object) value);
  }

  public static ClienteRemoto.ClienteRem narrow (org.omg.CORBA.Object obj)
  {
    if (obj == null)
      return null;
    else if (obj instanceof ClienteRemoto.ClienteRem)
      return (ClienteRemoto.ClienteRem)obj;
    else if (!obj._is_a (id ()))
      throw new org.omg.CORBA.BAD_PARAM ();
    else
    {
      org.omg.CORBA.portable.Delegate delegate = ((org.omg.CORBA.portable.ObjectImpl)obj)._get_delegate ();
      ClienteRemoto._ClienteRemStub stub = new ClienteRemoto._ClienteRemStub ();
      stub._set_delegate(delegate);
      return stub;
    }
  }

  public static ClienteRemoto.ClienteRem unchecked_narrow (org.omg.CORBA.Object obj)
  {
    if (obj == null)
      return null;
    else if (obj instanceof ClienteRemoto.ClienteRem)
      return (ClienteRemoto.ClienteRem)obj;
    else
    {
      org.omg.CORBA.portable.Delegate delegate = ((org.omg.CORBA.portable.ObjectImpl)obj)._get_delegate ();
      ClienteRemoto._ClienteRemStub stub = new ClienteRemoto._ClienteRemStub ();
      stub._set_delegate(delegate);
      return stub;
    }
  }

}
